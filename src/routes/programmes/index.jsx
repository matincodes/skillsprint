    ...
    <div className=" bg-black flex justify-center">
      <div className="w-[90%] lg:w-[80%] flex flex-col items-center ">
        <div className=" mt-28 w-80   ">
          <img src={spack} className=" -mb-5 ml-7" />
          <h1 className="font-bold text-4xl text-center text-white ">
            Explore Our Programmes
          </h1>
        </div>
        {/* explore card */}
        <div className="w-full h-[302px] border-solid border-[1px] border-white mt-24 grid grid-cols-2 gap-5">
          <ExploreCard />
        </div>
        {/* Choose programme Category */}
        <div className="w-full flex flex-col items-center gap-7   mt-44">
          <h1 className="font-bold text-2xl text-white w-80 text-center lg:w-full">
            Choose programme Category
          </h1>
          <div className="  w-full overflow-hidden scrollbar-thin scrollbar-thumb-blue-500 scrollbar-track-gray-200">
            <div className="w-[1700px]  lg:w-full  flex justify-center gap-8  lg:flex-wrap">
              {buttonContent.map((items, index) => (
                <button
                  onClick={() => handleClick(index)}
                  key={index}
                  className={` ${activeButton === index ? `bg-[#121212] font-bold` : `font-normal border-solid border-[1px] border-[#808080] `}  text-sm px-5 py-4  text-white flex items-center justify-center rounded-2xl cursor-pointer `}
                >
                  {items}
                </button>
              ))}
        ...
        <div className=" pb-64 w-full mt-14 grid grid-cols-1 gap-20 md:gap-10 md:grid-cols-2">
          {programmData.map((items, index) => (
            <ProgrammCard key={index} image={items.image} />
          ))}
        </div>
      </div>
    </div>
